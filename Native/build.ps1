...
if (-not $SkipRust) {
    ...
    try {
        ...
        # Use --no-route to bypass cargo routing for local Windows builds
        $CargoArgs = @('--no-route', 'build', '--workspace')
...
if ($Test) {
    ...
    if (-not $SkipRust) {
        ...
        Push-Location $RustWorkspace
        try {
            $TestOutput = & cargo --no-route test --workspace 2>&1
            if ($LASTEXITCODE -ne 0) {
...
Write-BuildStep "Phase 6: Build Summary"
